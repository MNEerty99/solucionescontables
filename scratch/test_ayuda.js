...
(async () => {
  ...
  console.log("Navigating to http://localhost:3006/#/demo/ayuda ...");
  await page.goto('http://localhost:3006/#/demo/ayuda', { waitUntil: 'networkidle0' });
  ...
  const errorBoxText = await page.evaluate(() => {
    ...
    const errorDiv = divs.find(d => d.textContent.includes('Error al cargar la sección'));
